...
2. Continue with **Google**, **or** create / sign in with email and password (8+ characters).
3. You land on Live, signed in. Tap **Go live**. The API will not mint a host token without that session.

Host screens offer **Google + email only**. Apple and X are not shown. Google completes after `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` are set on Vercel (below). Until then, a tap explains those vars — no fake credentials are shipped.

Google can also complete on Grok sandbox hosts via the existing broker. That path is **not** how production Vercel social login works.
...
3. Google via the Grok broker on `*.vercel.app` cannot complete OAuth (the baked preview client only allows `*.grok-sandbox.com` callbacks). Production social login is this app's own Better Auth Google provider once `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` are set.
...
| `GROK_AUTH_*` | Optional | Sandbox-only Google via the Grok broker — not used for Vercel Google |
...
Hosts only need **Google**. Do **not** invent or commit these. Create the Google Cloud OAuth client, then paste the real values and **redeploy**.

**Callback URL** (Better Auth). Add it in Google Cloud Console:

| Provider | Production callback |
|---|---|
| Google | `https://yoan-claire-live.vercel.app/api/auth/callback/google` |

For a Vercel Preview URL `https://<preview-host>`, also add `https://<preview-host>/api/auth/callback/google`.

Google does not accept `*.vercel.app` wildcards. Add the first Preview URL after it exists, or test social on Production first.

Apple and X are **not** offered to hosts. Server-side Better Auth wiring can stay dormant (env-gated). `/api/auth/status` may still report `providers.apple` / `providers.twitter` as false.
...
Native login on branch `cursor/android-eventview-a8b1` is email/password only (Retrofit → `/api/auth/sign-in/email`). Web hosts see Google + email. `/api/auth/status` still includes `providers.apple|google|twitter` and `socialMethods`; apple/twitter stay false unless those env vars exist. Until native Google is wired, hosts can use email/password in the native app, or Google / email on the website.
