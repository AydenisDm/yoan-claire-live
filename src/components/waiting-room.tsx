...
export function WaitingRoom({ status = "waiting" }: { status?: "waiting" | "reconnecting" }) {
  const title = status === "reconnecting" ? "Reconnecting" : "The ceremony will begin shortly";
  const copy = status === "reconnecting"
    ? "The live feed dropped for a moment. Holding this page open — we will restore the picture automatically."
    : "You are in the right place. Leave this page open. When the ceremony goes live, the picture will appear here.";
  return (
    <div className="flex aspect-video w-full flex-col items-center justify-center gap-4 bg-raised px-6 text-center">
      ...
      <p className="text-xs tracking-widest text-subtle uppercase">{eventConfig.coupleNames}</p>
    </div>
  );
}
